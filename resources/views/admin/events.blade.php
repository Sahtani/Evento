                    ...
                    <tbody class="bg-white">
                        ...
                        <tr>
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full"
                                            src="{{ asset('storage/uploads/' . $event->image) }}"
                                            alt="" />
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="flex items-center">
                                    <div class="ml-4">
                                        <div class="text-sm leading-5 text-gray-500">{{ $event->location }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="flex items-center">
                                    <div class="ml-4">
                                        <div class="text-sm leading-5 text-gray-500">{{ $event->category->name }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('admin.validateEvent', $event->id) }}">
                                    @method('PATCH')
                                    @csrf
                                    <button type="submit" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Accepte</button>
                                </form>
                            </td>
